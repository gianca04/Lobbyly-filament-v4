{{-- Partial for the Item INPUT form --}}
<div class="mv-section" id="section-input">
    <div class="mv-table-container">
        <table class="mv-table">
            <thead>
                <tr>
                    <th>Item</th>
                    <th>Location</th>
                    <th style="width: 120px;">Quantity</th>
                    @if($mode === 'create')
                        <th style="width: 80px; text-align: center;">Action</th>
                    @endif
                </tr>
            </thead>
            <tbody id="input-distributions">
                {{-- Dynamic rows inserted via JS --}}
            </tbody>
        </table>
    </div>
    
    @if($mode === 'create')
        <button type="button" class="mv-add-btn" id="input-add-row" style="margin-top: 1rem;">
            <span>+</span> Add another item
        </button>
    @endif
</div>

{{-- Template for input rows --}}
<template id="tpl-input-row">
    <tr data-row>
        <td>
            <div class="mv-field">
                <select data-field="item_id">
                    <option value="">Select item...</option>
                </select>
            </div>
        </td>
        <td>
            <div class="mv-field">
                <select data-field="location_id">
                    <option value="">Select location...</option>
                </select>
            </div>
        </td>
        <td>
            <div class="mv-field">
                <input type="number" data-field="quantity" min="1" step="1" value="1">
            </div>
        </td>
        <td style="text-align: center; width: 80px;">
            <button type="button" class="mv-row-remove" title="Remove">&times;</button>
        </td>
    </tr>
</template>
